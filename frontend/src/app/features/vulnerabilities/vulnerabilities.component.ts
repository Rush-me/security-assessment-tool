...
export class VulnerabilitiesComponent implements OnInit, OnDestroy {
  ...
  defaultVulnerabilities: Vulnerability[] = [
    {
      id: 1,
      vulnerabilityId: 1,
      ...
      cveScore: 7.4,
      overallLevel: 'High',
      vulnerabilityDescription: 'TLS 1.0 is enabled on the /api/v1/payments endpoint, allowing BEAST and POODLE attacks that can decrypt cardholder data in transit.',
      trackingId: 'Open',
    ...
    },
    {
      id: 2,
      vulnerabilityId: 2,
      vulnerabilityName: 'SQL injection in transaction search',
      vulnerabilityFamily: 'Data Validation',
      cveScore: 9.8,
      overallLevel: 'Critical',
      vulnerabilityDescription: 'SQL injection vulnerability in transaction search parameter allows unauthorized database query execution.',
      trackingId: 'Open',
      supportingAssets: [{ id: 2, assetName: 'Transaction Database' }] as any
    },
    {
      id: 3,
      vulnerabilityId: 3,
      vulnerabilityName: 'Weak session token entropy',
      vulnerabilityFamily: 'Authentication',
      cveScore: 5.5,
      overallLevel: 'Moderate',
      vulnerabilityDescription: 'Session tokens generated by the application have low entropy, making them vulnerable to brute-force or prediction attacks.',
      trackingId: 'Mitigated',
      supportingAssets: [{ id: 3, assetName: 'Auth Service' }] as any
    },
    {
      id: 4,
      vulnerabilityId: 4,
      vulnerabilityName: 'Missing rate limiting on auth endpoint',
      vulnerabilityFamily: 'Authentication',
      cveScore: 8.0,
      overallLevel: 'High',
      vulnerabilityDescription: 'The login and password reset endpoints do not implement rate limiting, exposing them to brute force and credential stuffing attacks.',
      trackingId: 'Open',
      supportingAssets: [{ id: 3, assetName: 'Auth Service' }] as any
    },
    {
      id: 5,
      vulnerabilityId: 5,
      vulnerabilityName: 'Plaintext credentials in config file',
      vulnerabilityFamily: 'Configuration',
      cveScore: 9.0,
      overallLevel: 'Critical',
      vulnerabilityDescription: 'Database passwords and API keys are stored in plaintext within the application configuration files.',
      trackingId: 'Open',
      supportingAssets: [{ id: 4, assetName: 'Configuration Server' }] as any
    },
    {
      id: 6,
      vulnerabilityId: 6,
      vulnerabilityName: 'Insufficient logging on admin panel',
      vulnerabilityFamily: 'Access Control',
      cveScore: 3.2,
      overallLevel: 'Low',
      vulnerabilityDescription: 'Admin panel activities, including login attempts and configuration changes, are not logged, hindering security incident detection and response.',
      trackingId: 'Accepted',
      supportingAssets: [{ id: 5, assetName: 'Admin Web Portal' }] as any
    }
  ];
    ...
    this.vulnForm.valueChanges.subscribe(val => {
      ...
      if (val.supportingAssets) {
        selected.supportingAssets = val.supportingAssets.map((id: number) => {
          ...
          return found ? found : { id, assetName: 'Payment API' };
        });
      }
  ...
  loadData(): void {
    const proj = this.activeProject();
    if (!proj?.id) {
      this.vulnerabilities.set(this.defaultVulnerabilities);
      this.selectVulnerability(this.defaultVulnerabilities[0]);
      return;
    }
